...
public class ProductGroupService {
    ...
    public List<OUProductGroup> getProductGroupBymat_group(String mat_group) throws ClassNotFoundException, SQLException, NamingException {
        conn = ConnectDB.getConnectionPGCA();
        String sql = generatesqlProductGroupBymat_group(mat_group);
        ps = conn.prepareStatement(sql);
        rs = ps.executeQuery();
        List<OUProductGroup> list = new ArrayList<OUProductGroup>();
        while (rs.next()) {
    ...
    private String generatesqlProductGroupBymat_group(String mat_group) throws ClassNotFoundException, SQLException, NamingException {
        String sql = "select a.*,b.mat_group from bm_product_class a  inner join sap_mara_mat_all b on a.MAT_NO = b.MAT_NO where a.SIZE_ID !=  '----' and a.PRODUCT_GROUP_ID  in ('" + mat_group + "')";
        return sql;
    }
}
